...
export async function addUserIfNotExist(knex: Knex, name: string) {
  await knex.raw(
    `DO $$ BEGIN IF NOT EXISTS ( SELECT FROM pg_catalog.pg_roles WHERE rolname = '${name}' ) THEN CREATE USER ${name}; END IF; END $$;`,
  );
}
